...
def run(
    ...
    **overrides,
    ...
    if cfg.model.get("freeze_world_model", False):
        jepa.eval()
        xy_head.eval()
        for p in jepa.parameters():
            p.requires_grad_(False)
        for p in xy_head.parameters():
            p.requires_grad_(False)
        logger.info("freeze_world_model=True: JEPA and xy probe are fixed/eval-only")
    ...
    for epoch in range(start_epoch, cfg.optim.epochs):
        ...
        for idx, (x, a, loc, _, _) in pbar:
            ...
            if value_coeff:
                value_mode = cfg.model.get("value_mode", "endpoint_td")
                ...
                with autocast(device.type, enabled=use_amp, dtype=dtype):
                    if value_mode == "multi_horizon_td":
                        horizons = [
                            int(h)
                            for h in cfg.model.get("value_horizons", [1, 2, 4, 8, 16])
                        ]
                        pairs_per_h = int(cfg.model.get("value_pairs_per_horizon", 0) or 0)
                        loss_terms = []
                        for h in horizons:
                            if h < 1 or h >= Tw:
                                continue
                            n_pairs = Tw - h
                            if pairs_per_h > 0 and pairs_per_h < n_pairs:
                                t_idx = torch.randperm(n_pairs, device=device)[:pairs_per_h]
                            ...
                            z_real_t = z_gt.index_select(2, t_idx).detach()
                            z_real_next = z_gt.index_select(2, next_idx).detach()
                            z_roll_t = z_roll.index_select(2, t_idx).detach()
                            z_roll_next = z_roll.index_select(2, next_idx).detach()
                            g = z_gt.index_select(2, goal_idx).detach()

                            done = torch.full(
                                (z_real_t.shape[0], z_real_t.shape[2]),
                                1.0 if h == 1 else 0.0,
                                device=device,
                                dtype=z_real_t.dtype,
                            )
                            with torch.no_grad():
                                target_real = done + value_gamma * (1.0 - done) * value_target(
                                    z_real_next, g
                                )
                                target_roll = done + value_gamma * (1.0 - done) * value_target(
                                    z_roll_next, g
                                )
                            loss_terms.append(
                                torch.nn.functional.mse_loss(
                                    value_head(z_real_t, g), target_real
                                )
                            )
                            loss_terms.append(
                                torch.nn.functional.mse_loss(
                                    value_head(z_roll_t, g), target_roll
                                )
                            )
                        if not loss_terms:
                            raise ValueError(
                                f"value_mode=multi_horizon_td has no valid horizons for T={Tw}"
                            )
                        value_loss = value_coeff * torch.stack(loss_terms).mean()
                    else:
                        ...
                        with torch.no_grad():
                            ...
                            td_target = done + value_gamma * (1.0 - done) * v_next
