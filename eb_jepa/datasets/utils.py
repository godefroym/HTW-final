...
def _resolve_env(env_name):
    ...
    if env_name == "two_rooms":
        ...
        return DynamicMazeDataset, DynamicMazeDatasetConfig, DynamicMazeNormalizer
    else:
        raise ValueError(
            f"Unknown env_name={env_name!r}; expected 'two_rooms', 'maze', or 'dynamic_maze'"
        )
...
def create_env(env_name, config, **kwargs):
    ...
    if env_name == "two_rooms":
        ...
        from eb_jepa.datasets.dynamic_maze.dynamic_maze import DynamicMazeEnv

        return DynamicMazeEnv(config=config, **kwargs)
    else:
        raise ValueError(
            f"Unknown env_name={env_name!r}; expected 'two_rooms', 'maze', or 'dynamic_maze'"
        )
...
def _init_gpu_stream(env_name, merged_cfg, config, chunk_size, device, dtype,
    ...
    if env_name == "two_rooms":
        ...
        raise NotImplementedError(
            "dynamic_maze does not have a GPU stream generator yet; use pipeline.backend=cpu"
        )
    else:
...
def init_data(env_name, cfg_data=None, device=None, **kwargs):
    ...
    Supported envs: ``two_rooms``, ``maze``, ``dynamic_maze``.
    ...
    if mode == "stream":
        ...
        if backend == "gpu":
            ...
            if env_name in ("maze", "dynamic_maze"):
