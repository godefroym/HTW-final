...
def _resolve_dataset(env_name):
    ...
    if env_name == "two_rooms":
        ...
        from eb_jepa.datasets.dynamic_maze.dynamic_maze import (
            DynamicMazeDataset as DatasetClass,
        )
        from eb_jepa.datasets.dynamic_maze.dynamic_maze import (
            DynamicMazeDatasetConfig as ConfigClass,
        )
    else:
        raise ValueError(
...
def _generate_part(seed, n_samples, dtype_name=None):
    ...
    if hasattr(_WORKER_DATASET, "_rng"):
        _WORKER_DATASET._rng = np.random.default_rng(seed)
